...
public class StringConsumer {
	...
	public StringEatResult eatStrings(String... strings) {
		...
		for (int i = 0; i < strings.length; i++) {
			if (getOriginalString().startsWith(strings[i], getPointer())) {
				...
				StringEatResult stringEatResult = new StringEatResult(true, strings, i);
	...
	public PatternEatResult eatPattern(Pattern... patterns) {
		...
		for (int i = 0; i < patterns.length; i++) {
			...
			if (matcher.lookingAt()) {
				...
				PatternEatResult patternEatResult = new PatternEatResult(true, patterns, i, matcher);
				setLastEatResult(patternEatResult);
				return patternEatResult;
			}
		}
		PatternEatResult patternEatResult = new PatternEatResult(false, patterns);
		setLastEatResult(patternEatResult);
		return patternEatResult;
	}
	 ...
	 * @return 消耗过的字符串。
	 */
	public String getComsumedString() {
		return getOriginalString().substring(0, getPointer());
	}
	...
	public void setLastEatCount(int lastEatCount) {
		if ((lastEatCount < 0 && lastEatCount != -1) || lastEatCount > getPointer()) {
			throw new IndexOutOfBoundsException();
		}
	...
	public String getLastEatString() {
		...
		return getOriginalString().substring(getPointer() - getLastEatCount(), getPointer());
	}
	...
	public class StringEatResult extends EatResult {
		 ...
		 *            匹配的字符串索引位置。
		 */
		public StringEatResult(boolean isSuccess, String[] strings, int matchStringIndex) {
			super(isSuccess);
			this.strings = strings;
		 ...
		 *            所有字符串。
		 */
		public StringEatResult(boolean isSuccess, int eatCount, String eatString, String[] strings) {
	...
	public class PatternEatResult extends EatResult {
		 ...
		 *            匹配器。
		 */
		public PatternEatResult(boolean isSuccess, Pattern[] patterns, int matchPatternIndex, Matcher matcher) {
			super(isSuccess);
			this.patterns = patterns;
		 ...
		 *            所有正则表达式。
		 */
		public PatternEatResult(boolean isSuccess, int eatCount, String eatString, Pattern[] patterns) {
			super(isSuccess, eatCount, eatString);
			this.patterns = patterns;
			this.matchPatternIndex = -1;
			this.matcher = null;
